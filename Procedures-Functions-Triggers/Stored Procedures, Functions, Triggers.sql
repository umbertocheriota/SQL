...
--«адание 1. 
--Ќапишите функцию, котора€ принимает на вход название должности (например, стажер), а также даты периода поиска, 
...
SELECT hr.vac_foo('стажер', '2020-01-01', '2021-01-01')


--«адание 2. 
--Ќапишите триггер, срабатывающий тогда, когда в таблицу position добавл€етс€ значение grade, которого нет в таблице-справочнике grade_salary. 
--“риггер должен возвращать предупреждение пользователю о несуществующем значении grade.
...
VALUES (4592, 'BI Engineer', 'ѕроизводственный', 100, 20, 5, 1)

DELETE FROM "position" WHERE pos_id >= 4592



--«адание 3. —оздайте таблицу employee_salary_history с пол€ми:
...
--last_update - текуща€ дата и врем€

--Ќапишите триггерную функцию, котора€ срабатывает при добавлении новой записи о сотруднике 
--или при обновлении значени€ salary в таблице employee_salary, и заполн€ет таблицу employee_salary_history данными.
...
CREATE TABLE employee_salary_history (
	...
	salary_old int,
	salary_new int,
	difference int, --GENERATED ALWAYS AS (salary_new - salary_old) STORED, --так и не смог нагуглить как использовать GENERATED ALWAYS в самой функции
...
--«адание 4. 
--Ќапишите процедуру, котора€ содержит в себе транзакцию на вставку данных в таблицу employee_salary. 
--¬ходными параметрами €вл€ютс€ пол€ таблицы employee_salary.
